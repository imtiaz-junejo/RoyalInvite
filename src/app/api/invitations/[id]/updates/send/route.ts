...
function buildInvitationUpdateMessage(invitation: { title: string; brideName: string; groomName: string }, message: string) {
  return [
    ...
    message.trim(),
    "",
    "Sent from the official wedding invitation number.",
  ].join("\n")
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const invitation = await prisma.invitation.findFirst({
      ...
      select: { id: true, title: true, brideName: true, groomName: true },
    })
    ...
    const recipients = await prisma.rsvp.findMany({
      where: {
        ...
        guestWhatsapp: { not: null },
      },
      select: { id: true, guestWhatsapp: true },
    })

    if (recipients.length === 0) {
      return NextResponse.json({ error: "No WhatsApp recipients opted in for this invitation" }, { status: 400 })
    }
    ...
    for (const recipient of recipients) {
      const log = await prisma.whatsAppMessageLog.create({
        data: {
          invitationId: invitation.id,
          rsvpId: recipient.id,
          messageType: "manual_update",
          recipient: recipient.guestWhatsapp || "",
          message: text,
          status: "pending",
        },
      })
      ...
      try {
        ...
        await prisma.rsvp.update({
          ...
          data: { lastUpdateSentAt: new Date() },
        })
        sent += 1
      } catch (err: any) {
        await prisma.whatsAppMessageLog.update({
